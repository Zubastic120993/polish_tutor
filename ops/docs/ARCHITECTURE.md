...
| --- | --- | --- |
| Orchestration flow | No explicit pipeline linking AI → DB → TTS | Document the sequence above and keep it updated in `ARCHITECTURE.md`; use `ops/docs/diagrams/` for exported PNGs if needed. |
...
## Next Steps
1. Translate the mermaid diagram into a saved asset (`ops/docs/diagrams/orchestration-seq.mmd`) so it can be rendered automatically in docs tooling.
