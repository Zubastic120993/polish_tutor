    ...
    try:
        ...
        async def _run_murf_flow() -> Path:
            try:
                ...
                synthesis_result = await murf_client.synthesize_speech(
                    text=text,
                    voice_id=voice_id,
                    language=language,
                    style=style,
                ...
                murf_job_id_local = synthesis_result["job_id"]
                job.meta["murf_job_id"] = murf_job_id_local
                job.meta["progress"] = "waiting_for_murf"
                job.save_meta()
                job_logger.info(
                    "Murf job submitted successfully",
                    extra={"murf_job_id": murf_job_id_local, "cache_key": cache_key},
                )

                job_logger.info(
                    "Waiting for Murf job completion",
                    extra={"murf_job_id": murf_job_id_local, "max_wait_time": 300},
                )

                await murf_client.wait_for_completion(
                    murf_job_id_local, poll_interval=2.0, max_wait_time=300.0
                )
                ...
                downloaded = await murf_client.download_audio(
                    murf_job_id_local, cache_path
                )
                return murf_job_id_local, downloaded
            finally:
                try:
                    await murf_client.client.aclose()
                except Exception:
                    pass

        murf_job_id, downloaded_path = asyncio.run(_run_murf_flow())
