...
class ChatRespondRequest(BaseModel):
    ...
    speed: Optional[float] = Field(
        1.0, description="Audio playback speed (0.75 or 1.0)", ge=0.75, le=1.0
    )
    confidence: Optional[int] = Field(
        None, description="Confidence slider value (1-5)", ge=1, le=5
    )
...
class LessonCatalogEntry(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="Lesson status (draft, in_progress, complete)"
    )
...
class LessonCatalogData(BaseModel):
    ...
    entries: List[LessonCatalogEntry] = Field(
        default_factory=list, description="Flattened lesson catalog entries"
    )
...
class SettingsUpdateRequest(BaseModel):
    ...
    voice_mode: Optional[str] = Field(
        None,
        description="Voice mode: 'offline' or 'online'",
        pattern="^(offline|online)$",
    )
    audio_speed: Optional[str] = Field(
        None,
        description="Audio speed: 'slow', 'normal', or 'fast'",
        pattern="^(slow|normal|fast)$",
    )
    translation: Optional[str] = Field(
        None,
        description="Translation mode: 'show', 'hide', or 'smart'",
        pattern="^(show|hide|smart)$",
    )
    mic_mode: Optional[str] = Field(
        None, description="Microphone mode: 'tap' or 'hold'", pattern="^(tap|hold)$"
    )
    tutor_mode: Optional[str] = Field(
        None,
        description="Tutor mode: 'coach', 'drill', or 'teacher'",
        pattern="^(coach|drill|teacher)$",
    )
    ...
    audio_output: Optional[str] = Field(
        None,
        description="Audio output: 'speakers' or 'headphones'",
        pattern="^(speakers|headphones)$",
    )
    theme: Optional[str] = Field(
        None,
        description="UI theme: 'light', 'dark', or 'dyslexia'",
        pattern="^(light|dark|dyslexia)$",
    )
    language: Optional[str] = Field(None, description="UI language")
    confidence_slider: Optional[int] = Field(
        None, description="Confidence slider value (1-5)", ge=1, le=5
    )
    profile_template: Optional[str] = Field(
        None,
        description="Profile template: 'kid', 'adult', or 'teacher'",
        pattern="^(kid|adult|teacher)$",
    )
