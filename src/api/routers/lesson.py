...
from src.api.schemas import LessonCatalogResponse, LessonGetResponse, LessonOptionsResponse
...
async def lesson_generate(request: LessonGenerateRequest):
    ...
    try:
        ...
        # Generate lesson
        logger.info(f"🎓 Generating lesson: topic='{request.topic}', level={request.level}, dialogues={request.num_dialogues}")
