...
async def user_stats(user_id: int = Query(..., description="User ID", gt=0)):
    ...
    try:
        ...
        recent_attempts = sorted(attempts, key=lambda x: x.created_at if x.created_at else datetime.min, reverse=True)[:10]
        accuracy_trend = [
            {
                "date": attempt.created_at.isoformat() + "Z" if attempt.created_at else datetime.utcnow().isoformat() + "Z",
