...
def verify_api_schema():
    ...
    response = PracticePackResponse(
        pack_id="test_pack", session_id=123, review_phrases=[], new_phrases=[]
    )
...
def verify_endpoint_integration():
    ...
    assert (
        "session_id=user_session.id" in endpoint_code or "session_id" in endpoint_code
    ), "session_id not returned"
